...
import br.com.easynet.gwt.easyportal.client.display.easyEngenharia.easyconstru.eqc_equivalencia_composicao.Eqc_equivalencia_composicaoInsertGWT;
import br.com.easynet.gwt.easyportal.client.display.easyEngenharia.easyconstru.eqc_equivalencia_composicao.Eqc_equivalencia_composicaoUpdateDeleteGWT;
 ...
 * @author marcos
 */
public class EquivalenciaComGWT extends Window implements IListenetResponse {
    ...
    final ComboBox<Con_composicao_novaTGWT> comboPlc = new ComboBox<Con_composicao_novaTGWT>();
    private List<Con_composicao_novaTGWT> listaPlc = new ArrayList<Con_composicao_novaTGWT>();
    ListStore<Con_composicao_novaTGWT> storeCombo = new ListStore<Con_composicao_novaTGWT>();
    ...
    final EquivalenciaComGWT consultGWT = this;
    ...
    final ListStore<Coa_composicao_anteriorTGWT> store2 = new ListStore<Coa_composicao_anteriorTGWT>();
    ContentPanel cpPlco = new ContentPanel();
    Grid<Coa_composicao_anteriorTGWT> grid;
    private ComboBox<Con_composicao_novaTGWT> comboFilterPlc = new ComboBox<Con_composicao_novaTGWT>();
    private ListStore<Coa_composicao_anteriorTGWT> store = new ListStore<Coa_composicao_anteriorTGWT>();
    private StoreFilterField<Coa_composicao_anteriorTGWT> storefilter;
    private ToolBar toolBarTop = new ToolBar();
    private FormPanel formbinding = new FormPanel();
    private TextArea areaplco = new TextArea();
    ListCoaGWT listCoaGWT = new ListCoaGWT();
//    ListInnGWT listInnDaoGWT = new ListInnGWT();
    ...
    public EquivalenciaComGWT() {
        this.setHeading("Equivalêcias Composição");

        this.listCoaGWT.povoa();

//        listInnDaoGWT.povoa();
        setSize(950, 490);
        mapEquivalencia = new HashMap<String, String>();

        storefilter = new StoreFilterField<Coa_composicao_anteriorTGWT>() {

            @Override
            protected boolean doSelect(Store<Coa_composicao_anteriorTGWT> store, Coa_composicao_anteriorTGWT parent, Coa_composicao_anteriorTGWT record, String property, String filter) {
                String name = record.get("coa_tx_descricao");
        ...
        button.addSelectionListener(new SelectionListener<ButtonEvent>() {
            ...
            public void componentSelected(ButtonEvent ce) {
                ...
                String page = Constantes.URL+"easyEngenharia/easyconstru/coa_composicao_anterior/gerar_xlsComposicao.jsp?op=gerarArquivoXls";
        ...
        KeyListener kl = new KeyListener() {
            ...
            public void componentKeyPress(ComponentEvent event) {
                if (event.getKeyCode() == KeyCodes.KEY_ENTER) {
                    if (tx_locate.getValue() != null) {
                        loadCon(tx_locate.getValue());
                    }
                }
            }
        };
        ...
        KeyListener kl2 = new KeyListener() {
            ...
            public void componentKeyPress(ComponentEvent event) {
                if (event.getKeyCode() == KeyCodes.KEY_ENTER) {
                    if (tx_locate2.getValue() != null) {
                        loadCon2(tx_locate2.getValue());
                    }
                }
            }
        };
        tx_locate2.setEmptyText("Inf. parte do nome");
        tx_locate2.setWidth(120);
        tx_locate2.addKeyListener(kl2);
        montar();
    }

    public void loadCon(String descricao) {
        final ListConGWT listConGWT = new ListConGWT();
        listConGWT.povoa(descricao);
        Timer timer = new Timer() {

            @Override
            public void run() {
                if (!listConGWT.povoado) {
                    schedule(500);
                } else {
                    comboPlc.setEmptyText("selecione insumo");
                    comboPlc.getStore().removeAll();
                    comboPlc.getListView().getStore().removeAll();
                    comboPlc.getListView().getStore().add(listConGWT.list);
                    comboPlc.getListView().refresh();
                    if (listConGWT.list.size() == 1) {
                        comboPlc.setValue(listConGWT.list.get(0));
                    } else {
    ...
    public void loadCon2(String descricao) {
        final ListConGWT listConGWT = new ListConGWT();
        listConGWT.povoa(descricao);
        Timer timer = new Timer() {
            ...
            public void run() {
                if (!listConGWT.povoado) {
                    ...
                    comboFilterPlc.getStore().add(listConGWT.list);
                    comboFilterPlc.getListView().refresh();
                    comboFilterPlc.getListView().getStore().removeAll();
                    comboFilterPlc.getListView().getStore().add(listConGWT.list);
                    if (listConGWT.list.size() == 1) {
                        comboFilterPlc.setValue(listConGWT.list.get(0));
                    } else {
                    ...
                    Timer timerPlc = new Timer() {
                        ...
                        public void run() {
                            if (!vali) {
                                ...
                                ListStore<Con_composicao_novaTGWT> list = new ListStore<Con_composicao_novaTGWT>();
                                //list.add(listInnDaoGWT.list);
                                comboFilterPlc.setStore(list);
                                comboFilterPlc.setDisplayField("con_tx_descricao");
                                ...
                                Grid<Coa_composicao_anteriorTGWT> grid2 = new Grid<Coa_composicao_anteriorTGWT>(store2, createColumnModel());
                                grid2.setBorders(true);
                                grid2.getColumnModel().getColumn(0).setHeader("Composição da controladoria");
                                ...
                                storeCombo = new ListStore<Con_composicao_novaTGWT>();

//                                storeCombo.add(listInnDaoGWT.list);

                                comboPlc.setStore(storeCombo);
                                comboPlc.setDisplayField("con_tx_descricao");
                                comboPlc.setWidth(300);
                                comboPlc.setEmptyText("Selecione a composição");
                                ...
                                button.addListener(Events.OnClick, new Listener<ButtonEvent>() {
                                    ...
                                    public void handleEvent(ButtonEvent be) {
                                        ...
                                        for (Coa_composicao_anteriorTGWT coaT : store2.getModels()) {
                                            param += "idsCoa=" + coaT.getCoa_nr_id() + "&";
                                        }
    ...
    private void atualizarCpPlco() {
        cpPlco.removeAll();
        store.removeAll();
        store.add(listCoaGWT.list);

        grid = new Grid<Coa_composicao_anteriorTGWT>(store, createColumnModel());
...
//        });
    ...
    public void load() {
        final ListCoaGWT listCoaGWT = new ListCoaGWT();
        listCoaGWT.povoaEquiv(comboFilterPlc.getValue().getCon_nr_id());
        Timer timer = new Timer() {
            ...
            public void run() {
                if (!listCoaGWT.povoado) {
                    ...
                    store.add(listCoaGWT.list);
                    Grid<Coa_composicao_anteriorTGWT> gridEquivalencia = new Grid<Coa_composicao_anteriorTGWT>(store, cm);
    ...
    public void addEquivalencia(String param) {
        try {
            param += "idCon=" + comboPlc.getValue().getCon_nr_id() + "&";
            retornoEquivalencia retorno = new retornoEquivalencia();
            EasyAccessURL eaurl = new EasyAccessURL(retorno);
            eaurl.accessJSon(Constantes.URL + Eqc_equivalencia_composicaoInsertGWT.PAGE + param);
        } catch (Exception e) {
        }
    }
                ...
                Button btnCol = new Button("", new SelectionListener<ButtonEvent>() {
                    ...
                    public void componentSelected(ButtonEvent ce) {
                        try {
                            ...
                            param.put("op", "delete");
                            param.put("eqc_equivalencia_composicaoT.coa_nr_id", model.getCoa_nr_id() + "");
                            eaurl.accessJSonMap(Constantes.URL + Eqc_equivalencia_composicaoUpdateDeleteGWT.PAGE, param);
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
                    }
...
//            gridEquivalencia.setStyleAttribute("borderTop", "none");
    ...
    class retornoEquivalencia implements IListenetResponse {
