...
<jsp:useBean id="men_menuJB" class="br.com.easynet.easyportal.menu.jb.Men_menuUpdateDeleteJB" scope="request"/>                                         
...
<input type="hidden" name="men_menuT.men_nr_id" value="${men_menuJB.men_menuT.men_nr_id}" />
<div align="center">     
<label class="errorMessage">${men_menuJB.msg}</label>
<h3>Dados do Menu</h3>
<table>
<tr>
    <td>
    ...
    <td>
        <select name="sis_sistemaT.sis_nr_id" />
            <c:forEach items="${men_menuJB.listSistemas}" var="item">
                <option value="${item.sis_nr_id}" ${men_menuJB.men_menuT.sis_nr_id==item.sis_nr_id?'selected':''}>${item.sis_tx_nome}</option>
            </c:forEach>
        </select>
    </td>
</tr>
...
<tr>
    ...
    <td>
        <select name="sis_sistemaTFiltro.sis_nr_id" onchange="setOpPortal('men_menu','consult', 'men_menuUpdateDelete.jsp')"/>
            <c:forEach items="${men_menuJB.listSistemas}" var="item">
...
<tr>
    <td colspan="2" align="center">
        <input type="button" value="Update"  onClick="setOpPortal('men_menu','update', 'men_menuUpdateDelete.jsp')" /> &nbsp;&nbsp;&nbsp;
        <input type="button" value="Delete"  onClick="setOpPortal('men_menu','delete', 'men_menuUpdateDelete.jsp')" /> &nbsp;&nbsp;&nbsp;
