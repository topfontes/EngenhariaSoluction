...
<jsp:useBean id="ccffJB" class="br.com.easynet.gwt.easyportal.jb.Cronograma_fisico_fin_atualizadoJB" scope="request"/>
<jsp:setProperty name="ccffJB" property="*"/>
<jsp:setProperty name="ccffJB" property="page" value="${pageContext}"/>
${ccffJB.execute}
<form>
    <img border="0" src="/engenharia/easyEngenharia/easyconstru/relatorios/cronograma_fisico_fin_atualizado.jsp?op=downloadImage"/>
    <table>
        <tr>
            <td colspan="2" style="font-family: tahoma;font-size: 16">
                <strong>Relatório Cronograma Físico-Financeiro Por Regime de Competência Reprogramado</strong>
            </td>
        </tr>
        <tr>
            <td width="20">
            ...
            <td>${ccffJB.cli_clienteT.cli_tx_nome}</td>
        </tr>
        <tr>
            <td width="20">
                OBRA:
            </td>
            <td>${ccffJB.obr_obrasT.obr_tx_nome}</td>
        </tr>
    </table>
    <hr>
    <br/>
    <table border="1" width="100%" style="font-family: tahoma;font-size: 12px;border-collapse: collapse" bordercolor="black" >
        <c:forEach var="item" items="${ccffJB.listServico}" varStatus="cont">
        ...
        <tr>
            <td colspan="5">
                ...
                <label>% Executado no período</label></br>
                <label>% Executado acumulado</label>

            </td>
            <td>
                <label><strong> <fmt:formatNumber value="${ccffJB.totalOrcamento}"pattern="#,##0.00;-#,##0.00" minFractionDigits="2" type="currency"/></label></strong></br>
            </td>
            <c:forEach var="item" items="${ccffJB.listTotalMeses}">
                <td align="right">
                    <label><strong>
